...
1. In Jellyfin, go to **Dashboard → Plugins → Repositories** and add this repository URL (one URL serves both Jellyfin 10.11 and 12 - your server installs the matching build automatically):

   ```
   https://raw.githubusercontent.com/Flowfin/jellyfin-plugin-sso/manifest-release/manifest.json
   ```

   To run the betas instead, add the beta channel's URL, which carries every build as it lands:
