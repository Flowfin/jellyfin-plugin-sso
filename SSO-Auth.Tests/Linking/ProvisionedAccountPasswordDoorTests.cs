...
public class ProvisionedAccountPasswordDoorTests
{
    ...
    [Fact]
    public void ARecordNamingAnAccountThatNowHoldsNoPasswordAtAll_ReadsAsHoldingItsOwn_AndDoesNotThrow()
    {
        // THE ARM THE EMPTY-PASSWORD GUARD IS, and it is a reachable state rather than a defensive one: an
        // administrator clearing an SSO-provisioned account's password from the Jellyfin dashboard leaves
        // the record naming an account whose stored password is now nothing. Without the guard the digest
        // is taken of a null and the read THROWS, which is a 500 on the caller's own unlink rather than an
        // answer. The answer is false: an empty password is an open door rather than a seal, which is the
        // state the boot-time pass exists to end.
        var (service, _, _, config) = BuildWithConfig();
        var user = TestUsers.Named("frank", Created);
        config.ProvisionedPasswords[user.Id] = "a-digest-of-a-hash-that-is-no-longer-there";
        user.Password = null;
