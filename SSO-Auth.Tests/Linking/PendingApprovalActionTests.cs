...
public class PendingApprovalActionTests
{
    ...
    [Fact]
    public async Task AClearThatCannotPersist_IsAWarning_AndNotAFailedLogin()
    {
        // AVAILABILITY. The clear runs after the session has been minted, so a configuration persist that
        // throws - a read-only or full volume being the ordinary way - must not turn a login that already
        // succeeded into an error the reader sees. The cost is the record standing until the approve
        // action refuses it on its own, and the warning names the provider and nothing that identifies
        // the account.
        var config = new OidConfig { Enabled = true };
        ...
        var linksLog = new CapturingLogger();
        var store = new ProviderConfigStore(() => configuration, _ => throw new System.IO.IOException("disk full"), new CapturingLogger());
        var canonicalLinks = new CanonicalLinkService(users, new FakeCryptoProvider(), store, linksLog, clock: () => Now);
        ...
        var service = new LoginCompletionService(canonicalLinks, minter, ssoOnly, store, sessions, new CapturingLogger());
        var user = TestUsers.Named("alice", Pending);
        users.GetUserById(Pending).Returns(user);
        users.GetUserByName("alice").Returns(user);
        config.CanonicalLinks["sub-1"] = Pending;
        ...
        var warning = Assert.Single(linksLog.Entries, e => e.Message.Contains("pending-approval record", StringComparison.Ordinal));
        ...
        Assert.DoesNotContain("sub-1", warning.Message, StringComparison.Ordinal);
        Assert.DoesNotContain("alice", warning.Message, StringComparison.Ordinal);
    }

    [Fact]
    public void TheLiveRule_RefusesAnEmptyKeyAndANullValue()
    {
        // The two shapes the dictionary itself would answer badly: an empty key is what an unresolved
        // identity carries and must name no record rather than throw, and a null value is what a hand-edited
        // configuration file can put in the map and must read as no record rather than dereference.
        var config = new OidConfig();
        config.CanonicalLinks["sub-1"] = Pending;
        config.CanonicalLinkPendingApprovals["sub-1"] = null!;

        Assert.Null(PendingApproval.Live(config, string.Empty));
        Assert.Null(PendingApproval.Live(config, null));
        Assert.Null(PendingApproval.Live(config, "sub-1"));
    }

    [Fact]
    public void AClearForABlankKey_WritesNothing()
    {
        // The login-time clear is keyed by the subject the login carried, and a login with no subject has no
        // record to clear: the blank key returns before the locked read rather than asking a dictionary
        // about whitespace, and the record under a real key is untouched.
        var (service, config, users) = Build();
        Recorded(users, config, disabled: true, admin: false);

        service.ClearPendingApprovalAfterLogin(ProviderMode.Oid, "kc", " ");
