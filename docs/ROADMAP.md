...
soaked for seven days as the newest beta with no release-blocking defect
reported, the manual checklist walked on real hardware, and the provider
matrix green against Jellyfin 12.0 and 12.1. It is safe for real instances
with real accounts; as with any auth change, keep a config backup.
