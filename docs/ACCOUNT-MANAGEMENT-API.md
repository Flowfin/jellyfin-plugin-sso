...
| An administrator account and an identity this instance does not already link to it | `that account is an administrator and this instance does not already link that identity to it; pre-provision the link deliberately, then import` | The two refusals above compare the file against a link this server already holds, and a rebuilt target holds none. Pre-provision that pairing on its own, then re-import |
...
| A provider whose `OidEndpoint` is missing or not a usable URL                      | `the OpenID endpoint configured for this provider is not a usable URL, so nothing says what it issues`                                           | Fail closed: a provider that can complete no login has no issuer that is right to store against it                                                                       |
