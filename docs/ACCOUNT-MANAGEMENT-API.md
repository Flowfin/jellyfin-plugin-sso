...
| A provider carrying `DoNotValidateIssuerName`                       | none - the entry is not checked                                                                                                                                                                      | The stored binding is the id_token `iss` and the fact read here is the discovery `issuer`; that flag exists for providers where the two differ permanently, so there is no comparand and the entry restores unchecked |
