...
| An issuer the provider is not configured to issue                   | `the file binds that link to issuer '<file>', but this provider is configured to issue '<configured>'; re-point the provider, or re-key the links deliberately, before importing`                    | The stored issuer is compared against the provider's discovery document at import time, because a stored issuer the provider does not issue refuses EVERY later login for that link                                                                                                                                                                                                        |
| A provider whose discovery document declares a TEMPLATED issuer     | none - the entry is not checked                                                                                                                                                                      | The stored binding is the id_token `iss` and the fact read here is the discovery `issuer`; a multi-tenant provider publishes a template in place of a value, so there is no comparand and the entry restores unchecked, with an `[SSO Audit]` line naming the provider. `DoNotValidateIssuerName` alone does not exempt: a provider setting it and declaring a concrete issuer is compared |
| An issuer read from an endpoint the provider no longer uses         | `this provider's OpenID endpoint changed while the import was being prepared, so what it issues was read from an endpoint it no longer uses; import again`                                           | The read happens before the configuration lock; a provider re-pointed in that window is a different identity provider                                                                                                                                                                                                                                                                      |
| An issuer that could not be compared at all                         | `the file binds that link to issuer '<file>', and what this provider issues could not be read to compare it against: <cause>`, or `... and nothing was read for this provider to compare it against` | Fail closed: writing an issuer nothing validated is the same outcome as writing a wrong one, so an unreadable discovery document refuses rather than storing it                                                                                                                                                                                                                            |
