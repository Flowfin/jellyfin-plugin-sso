...
- **The RP-initiated OpenID logout accepts a one-time ticket, so a client never
  ...
  code. The mint sits behind the existing `EnableSingleLogout` switch, so a server
  that never turned Single Logout on gains no new surface. The `api_key` form
  still works for a client that cannot mint. There is **no sign-out button yet**:
