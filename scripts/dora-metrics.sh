...
if (( span_days > 0 )); then
  ...
  echo "   Rate: n/a; release history spans <1 day, so frequency is not yet meaningful."
fi
echo "   Releases:"
...
if (( intervals == 0 )); then
  echo "   Only one stable release exists; no predecessor interval, so lead time is not computable yet."
...
if (( judgeable > 0 )); then
  ...
  echo "   CFR: n/a; only one stable release, so no release can yet be judged."
fi
