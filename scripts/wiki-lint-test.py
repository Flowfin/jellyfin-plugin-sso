...
class DeadSourceLinksHonourTheRef(unittest.TestCase):
    ...
    def test_a_ref_that_reads_as_an_option_is_a_finding_and_never_a_fetch(self) -> None:
        # The ref comes out of wiki text. Passed to git as it stands, a ref written as an option would
        # be read as one, so the check refuses it by name before anything is fetched; the message is
        # the one that tells this refusal from a ref origin does not serve.
        findings = self._findings("--upload-pack=touch%20owned/docs/on-both.md")
        self.assertEqual(1, len(findings), findings)
        self.assertIn("ref '--upload-pack=touch%20owned' is not a name this check fetches", findings[0])
        self.assertNotIn("could not be fetched", findings[0])
