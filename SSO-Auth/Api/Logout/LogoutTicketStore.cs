...
internal sealed class LogoutTicketStore
{
    ...
    // the request path (mirrors the siblings). Throttling only defers memory reclamation - the redeem
    // predicate rejects an expired ticket independently, so a not-yet-swept entry never completes a logout.
...
/// <param name="SessionToken">The minting caller's own Jellyfin access token, so the redeem ends THAT session and not merely some session of that user. Three guards keep it off every wire, and the third is there because the first two were measured and found short. <see cref="JsonIgnoreAttribute"/> covers System.Text.Json; <c>Newtonsoft.Json.JsonIgnoreAttribute</c> covers Newtonsoft, which is a direct dependency of this project and honours only its own attribute - measured on the shipped record, a Newtonsoft serialization wrote the live token out in full while the System.Text.Json one hid it; and the overridden <c>ToString</c> below covers interpolation and any log sink that destructures an object instead of calling a serializer. What is claimed is those three and no more: a serializer this plugin does not carry is not covered, and the guard cannot reach one.</param>
