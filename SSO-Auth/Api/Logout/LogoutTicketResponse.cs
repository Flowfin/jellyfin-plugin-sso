...
/// </para>
/// <para>
/// IT IS DECLARED TWICE, FOR THE REASON <see cref="Jellyfin.Plugin.SSO_Auth.Api.Logout.LogoutTicket"/>
/// CARRIES TWO IGNORE ATTRIBUTES. Newtonsoft is a direct dependency of this project and honours only its
/// own attribute, so a System.Text.Json declaration alone left the key inherited under the serializer the
/// record beside it names as a live hazard - measured, this response answered <c>{"ticket":...}</c> under
/// System.Text.Json and <c>{"Ticket":...}</c> under Newtonsoft while this paragraph said the key was the
/// type's own. Jellyfin serves its API with System.Text.Json, so the practical exposure was small and the
/// defect was the claim. The pinning row serializes under all three shapes now rather than the two.
/// </para>
/// </remarks>
