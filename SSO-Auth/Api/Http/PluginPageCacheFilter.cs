...
internal sealed class PluginPageCacheFilter : IResultFilter
{
    ...
    public void OnResultExecuting(ResultExecutingContext context)
    {
        ...
        try
        {
            ...
            {
                // The stream the host opened over the embedded resource will not be read.
    ...
    // A strong comparison: the tag is the whole version, and a weak match would validate an asset whose
    // bytes differ. `*` is a client asking for anything the server holds, which for a stored asset is the
    // current one.
    private static bool HoldsCurrentVersion(HttpRequest request)
    {
        return EntityTagHeaderValue.TryParseList(request.Headers.IfNoneMatch, out var tags)
            && tags.Any(tag => tag.Equals(EntityTagHeaderValue.Any) || tag.Compare(PluginAssetVersion.ETag, useStrongComparison: true));
    }
}
