            ...
            if (_logger.IsEnabled(LogLevel.Warning))
            {
                ...
                switch (OidcChallengeRefusal.Classify(state.Error))
                {
                    ...
                    case OidcChallengeCause.ClientAuthentication:
                        _logger.LogWarning("OpenID login refused for provider {Provider}: preparing the authorization request failed ({Error} - {ErrorDescription}). That is a refusal of the client rather than of the request: check the client ID and the client secret this provider is configured with, and whether a secret is sent here while the provider holds the client as public, or the reverse. This line never carries the provider's own words for a refusal, so the provider's log for this request is where a cause other than those would show.", provider?.ReplaceLineEndings(string.Empty).Replace('[', '('), state.Error?.ReplaceLineEndings(string.Empty).Replace('[', '('), state.ErrorDescription?.ReplaceLineEndings(string.Empty).Replace('[', '('));
                        break;

                    default:
                        _logger.LogWarning("OpenID login refused for provider {Provider}: preparing the authorization request failed ({Error} - {ErrorDescription}). This line does not interpret that answer, because the field it arrives in carries three different things: the provider's own error code, the HTTP status the request came back with, or the failure that stopped it arriving. Read it as whichever of the three it is.", provider?.ReplaceLineEndings(string.Empty).Replace('[', '('), state.Error?.ReplaceLineEndings(string.Empty).Replace('[', '('), state.ErrorDescription?.ReplaceLineEndings(string.Empty).Replace('[', '('));
                        break;
                }
            }
