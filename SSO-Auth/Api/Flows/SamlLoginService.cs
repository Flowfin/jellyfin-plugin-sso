...
internal sealed class SamlLoginService
{
    ...
    public async Task<ActionResult> AuthenticateAsync(string provider, AuthResponse response, string? bindingCookie, Func<string> remoteEndPointResolver)
    {
        ...
        // for one release, dropped by #528). That legacy body is NOT re-parsed or re-validated as an
        // assertion; it simply is not a live token, so it is rejected the same uniform way as any other
        // non-token, never falling through open. The removal is the wire-contract break #251 flagged: a
        // scripted client that POSTs a raw assertion straight to SAML/Auth (bypassing the rendered page) is
        // now rejected.
